        ...
        listOf(400, 1100).forEach { w ->
            ...
            try {
                runSkikoComposeUiTest(size = Size(w.toFloat(), 900f)) {
                    ...
                    waitUntil(timeoutMillis = 20_000) { onAllNodesWithTag(learningUnitCardTag(units[1].id)).fetchSemanticsNodes().isNotEmpty() }
                    shoot("01-topic")
                    // Unit 4: richest blocks (wide comparisons + code).
                    val unit = units[3]
